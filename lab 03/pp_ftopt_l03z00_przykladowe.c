...
- pobiera od uzytkownika dwa argumenty: wysokosc trojkata (liczba calkowita) oraz liczbe poczatkowa (liczba calkowita)
- rysuje na ekranie trójkąt prostokątny, równoramienny, wypełniony liczbami od poczatkowej kolejno co jeden do ostatniego znaku
- zwraca podana przez uzytkownika wysokosc w postaci liczby calkowitej.
...
void testy(){
    if(rysuj_trojkat(1,5) == 1){
        printf("1 -> pass\n\n");
    }
    else{
        printf("1 -> error!!!\n\n");
    }
    if(rysuj_trojkat(1, 7) == 1)
        printf("2 -> pass\n\n");
    else
        printf("2 -> error!!!\n\n");
    rysuj_trojkat(5, 8) == 5 ? printf("3 -> pass\n") : printf("3 -> error!!!\n");
}
