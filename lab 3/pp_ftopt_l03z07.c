...
Napisz funkcje iloczyn_rekurencyjnie(), która:
- pobiera od uzytkownika jeden argument - dodatnia liczbe calkowita
- oblicza w sposob rekurencyjny iloczyn kolejnych liczb rozpoczynajac od 1 i konczac na podanej przez uzytkownika liczbie (jest to silnia tej liczby)
...
void testy(){
    if(iloczyn_rekurencyjnie(3) == 6){
        printf("1 -> pass\n\n");
    }
    else{
        printf("1 -> error!!!\n\n %d\n", iloczyn_rekurencyjnie(3));
    }
    if(iloczyn_rekurencyjnie(5) == 120)
        printf("2 -> pass\n\n");
    else
        printf("2 -> error!!!\n\n");
    iloczyn_rekurencyjnie(4) == 24 ? printf("3 -> pass\n") : printf("3 -> error!!!\n");
}
