...
Napisz funkcje potega_iteracyjnie(), która:
- pobiera od uzytkownika jeden argument - dodatnia liczbe calkowita (wykladnik potegowania n)
- oblicza w sposob iteracyjny n-ta (posugujac sie liczba podana przez uzytkownika) potege liczby 2
...
void testy(){
    if(potega_iteracyjnie(3) == 8){
        printf("1 -> pass\n\n");
    }
    else{
        printf("1 -> error!!!\n\n %d\n", iloczyn_iteracyjnie(3));
    }
    if(potega_iteracyjnie(5) == 32)
        printf("2 -> pass\n\n");
    else
        printf("2 -> error!!!\n\n");
    potega_iteracyjnie(4) == 16 ? printf("3 -> pass\n") : printf("3 -> error!!!\n");
}
