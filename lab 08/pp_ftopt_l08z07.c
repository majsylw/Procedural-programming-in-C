...
- struct Polynomial gethigherPolynomial(struct Polynomial *, int);
pozwala na podniesienie stopnia podanego wielomianu, tzn. jako argumenty
podajemy wskaznik na wielomian oraz mnoznik n, w rezultacie nasz wielomian zostanie przemnozony przez x^n
np. dla 2x^2+3x+1x^1 oraz mnoznika n=2 dostaniemy 2x^4+3x^3+x^2+0x^1+0x^0
funkcja zwraca wielomian (zmienna typu struct Polynomial)
(aby zaalokowac ponownie miejsce na powiekszona tablice wspolczynnikow skorzystaj z funkcji realloc:
https://4programmers.net/C/Biblioteka_standardowa/Stdlib.h/Realloc)
